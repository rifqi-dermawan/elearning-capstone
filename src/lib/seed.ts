...
async function seed() {
  ...
  // ─── Concept Relations ────────────────────────────────
  const relations = [
    { sourceId: "m1", targetId: "m2", weight: 0.9 }, // HTML → JS
    { sourceId: "m2", targetId: "m6", weight: 0.95 }, // JS → React Hooks
    { sourceId: "m6", targetId: "m9", weight: 0.85 }, // React → TypeScript
    { sourceId: "m3", targetId: "m4", weight: 0.9 },  // DB → API
    { sourceId: "m5", targetId: "m7", weight: 0.95 }, // AI Fund → Neural Net
    { sourceId: "m5", targetId: "m8", weight: 0.9 },  // AI Fund → Data Science
    { sourceId: "m4", targetId: "m10", weight: 0.8 }, // API → Docker
  ...
  console.log(`✅ ${relations.length} concept relations seeded`);
  ...
  console.log(`✅ ${logs.length} user logs seeded`);

  console.log("\n🎉 Seeding complete! All users password: password123");
  console.log("\nUser accounts:");
  console.log("  admin@test.com  (ADMIN)");
  console.log("  andi@test.com   (Frontend profile)");
  console.log("  budi@test.com   (Backend profile)");
  console.log("  citra@test.com  (AI/DS profile)");
  console.log("  dani@test.com   (New user / cold start)");
  console.log("  eva@test.com    (Full-stack profile)");
}
